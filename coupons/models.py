...
class Coupon(models.Model):
    COUPON_TYPE_CHOICES = (
            ('product', 'Product Specific Coupon'),
            ('general', 'General / Referral Coupon'),
    )
    code = models.CharField(max_length=20, unique=True)
    coupon_type = models.CharField(max_length=20, choices=COUPON_TYPE_CHOICES, default='general')
    applicable_product = models.ForeignKey(Product, on_delete=models.SET_NULL, null=True, blank=True)
